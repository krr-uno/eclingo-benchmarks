...
clingo_re = {
    ...
    "int-time"     : ("float",  re.compile(r"^Time\s*:\s* (?P<val>[0-9]+(\.[0-9]+)?)s")),
    "sol-time"     : ("float",  re.compile(r"^Time\s*:\s*.*Solving: (?P<val>[0-9]+(\.[0-9]+)?)s")),
    "gr-time"      : ("float",  re.compile(r"^Time\s*:\s*.*Grounding: (?P<val>[0-9]+(\.[0-9]+)?)s")),
    "pre-time"     : ("float",  re.compile(r"^Time\s*:\s*.*Preprocesing: (?P<val>[0-9]+(\.[0-9]+)?)s")),
    "main-gr-time" : ("float",  re.compile(r"^\s*Main grounding time\s*:\s*(?P<val>[0-9]+(\.[0-9]+)?)s")),
    "gen-gr-time"  : ("float",  re.compile(r"^\s*Generator grounding time\s*:\s*(?P<val>[0-9]+(\.[0-9]+)?)s")),
    "tst-gr-time"  : ("float",  re.compile(r"^\s*Tester grounding time\s*:\s*(?P<val>[0-9]+(\.[0-9]+)?)s")),
    "wvb-gr-time"  : ("float",  re.compile(r"^\s*Worldview builder grounding time\s*:\s*(?P<val>[0-9]+(\.[0-9]+)?)s")),
    "candidates"   : ("float",  re.compile(r"^\s*Candidates\s*:\s*(?P<val>[0-9]+(\.[0-9]+)?)")),
    "tester-calls" : ("float",  re.compile(r"^\s*Tester calls\s*:\s*(?P<val>[0-9]+(\.[0-9]+)?)")),
}

# Candidates    : 1
# Tester calls  : 2

def eclingo(root, runspec, instance):
    """
